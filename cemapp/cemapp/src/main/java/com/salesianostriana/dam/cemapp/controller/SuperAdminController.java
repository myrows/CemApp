...
public class SuperAdminController {
	...
	public ResponseEntity<?> getAllUsuarios() {
		List<AppUser> usuarios = appUserService.findAll();
		
		//Creo las listas y el usuario dto
		List<UserDto> usuariosAMostrar = new ArrayList<UserDto>();
		...
		if(usuarios.isEmpty()) {
			...
			//Si hay, se recorre el findAll y por cada usuario se le setea al DTO vacío sus datos y se le añade a la lista
			for(AppUser usuario: usuarios) {
				UserDto usuarioAnyadir = new UserDto();
				
				usuarioAnyadir.setId(usuario.getId());
				usuarioAnyadir.setColegio(usuario.getColegio().getNombre());
				usuarioAnyadir.setEmail(usuario.getEmail());
				usuarioAnyadir.setRoles(usuario.getRoles().iterator().next());
				
				usuariosAMostrar.add(usuarioAnyadir);
